...
import numpy as np
import sys
sys.path.insert(0,"./timeIntegration")
import DeC
import RungeKutta
import ODEproblems
...
def run_model_and_simulation():
    ...
    st.latex(r"\frac{dS}{dt} = -\beta \frac{SI}{N}, \quad \frac{dI}{dt} = \beta \frac{SI}{N} -  \gamma I, \quad  \frac{dR}{dt}= \gamma I .")
...
def run_model():
    ...
    st.latex(r"\frac{dI}{dt} = \beta I \frac{S}{N} -\gamma I = \gamma \left( R_0 I \frac{S}{N} - I\right)= \gamma I \underbrace{\left( R_0 \frac{S}{N} - 1 \right)}_{\text{Segno?}} ")
