...
def app():
    ...
    st.write("I casi raffigurati sono relativi a 100,000 abitanti per ogni nazione. "
        "I decessi dipendono dal coefficiente di letalità settato.")
    ...
    st.write("Questo grafico ci racconta approsimativamente "
        "come si è modificata la letalità appartente.")
...
def plotCovidData(dataCovid, country, fatality=2, deathDelay=12, logScale=True):
    ...
    nuoviDecAveraged[idxsNan] = np.nan
    idxsNan = np.isnan(nuoviCasAveraged)
    nuoviCasAveraged[idxsNan] = np.nan
    ...
    source = source.append(
        pd.DataFrame({
            ...
            'Casi/decessi per 100k': np.array(nuoviCasAveraged),
            'Casi':np.array(nuoviCasAveraged),
            'Classe':'Casi su 100,000 abitanti'
    }))
    nmb=np.int(np.ceil(100000/fatality))
    source = source.append(
        pd.DataFrame({
        'Time':np.array(dateValuesDeaths),
        'Casi/decessi per 100k': np.array(nuoviDecAveraged),
        'Casi':np.array(nuoviDecAveraged)/fatality,
        'Classe':f'Decessi su {nmb:,} abitanti'
    }))

    skip=1
    letalApp=np.convolve(nuoviDecAveraged[deathDelay+skip:] / nuoviCasAveraged[skip:-deathDelay], 1 / 7 * np.ones(7), 'valid')
    idxsNan = np.isnan(letalApp)
    letalApp[idxsNan] = np.nan
    idxsTooBig = letalApp>0.2
    letalApp[idxsTooBig]=np.nan

    source2 = pd.DataFrame({})
    source2 = source2.append(
        pd.DataFrame({
            'Time':np.array(dateValuesCases[deathDelay+6+skip:]),
            'Frazione':letalApp,
    ...
    if logScale:
        ...
        alt.datum.Casi > 0  
            ...
            alt.Y('Casi' , scale=alt.Scale(type='log',domain=[0.1, 500])),
            x='Time',
